...
class ThingDataController {
	...
	set additionalType(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.additionalType; }
		else if(ThingDataController.isURL(value)){ this.model.additionalType = value}
        else{ ThingDataController.logError(this.type+"'s additionalType must be url"); }
	}

	get alternateName(){ return this.model.alternateName; }
	set alternateName(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.alternateName; }
		else if(ThingDataController.isString(value)){ this.model.alternateName = value}
		else{ ThingDataController.logError(this.type+"'s alternateName must be string"); }

	}

	get description(){ return this.model.description; }
	set description(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.description; }
		else if(ThingDataController.isString(value)){ this.model.description = value}
		else{ ThingDataController.logError(this.type+"'s description must be string"); }

	}

	get disambiguatingDescription(){ return this.model.disambiguatingDescription; }
	set disambiguatingDescription(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.disambiguatingDescription; }
		else if(ThingDataController.isString(value)){ this.model.disambiguatingDescription = value}
		else{ ThingDataController.logError(this.type+"'s disambiguatingDescription must be string"); }

	}

	get identifier(){ return this.model.identifier; }
	set identifier(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.identifier; }
		...
		else{ ThingDataController.logError(this.type+"'s identifier must be string, number or object"); }

	}

	get image(){ return this.model.image; }
	set image(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.image; }
		...
		else{ ThingDataController.logError(this.type+"'s image must be string, or object"); }
	...
	set mainEntityOfPage(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.mainEntityOfPage; }
		...
		else{ ThingDataController.logError(this.type+"'s mainEntityOfPage must be string, or object"); }

	}

	get name(){ return this.model.name; }
	set name(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.name; }
		else if(ThingDataController.isString(value)){ this.model.name = value}
		else{ ThingDataController.logError(this.type+"'s name must be string"); }
	}

	//TODO
	get potentialAction(){ return this.model.potentialAction; }
	set potentialAction(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.potentialAction; }
		...
		else{ ThingDataController.logError(this.type+"'s potentialAction must be string or object"); }
	}

	get sameAs(){ return this.model.sameAs; }
	set sameAs(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.sameAs; }
		else if(ThingDataController.isURL(value)){ this.model.sameAs = value}
		else{ ThingDataController.logError(this.type+"'s sameAs must be url"); }
	}

	get url(){ return this.model.url; }
	set url(value){
		if(ThingDataController.isNullOrUndefined(value)){ delete this.model.url; }
		else if(ThingDataController.isURL(value)){ this.model.url = value}
		else{ ThingDataController.logError(this.type+"'s url must be string"); }
	}
